...
class clevr_human_dataset_nmn(Dataset):
    def __init__(self, split, config, transform,max_ques_words=30,max_ops = 12, noop_idx = 0, eos='[SEP]'):
        self.split = split        
        if split == 'train':
            self.imdb = json.load(open(config['train_file']))['questions']
        elif split == 'val':
            self.imdb = json.load(open(config['val_file']))['questions']
        elif split == 'test':
            self.imdb = json.load(open(config['test_file']))['questions']
        ...
        self.clevr_root = config['clevr_root']
    ...
    def __getitem__(self, index):    
        ...
        ann = self.imdb[index]
        image_path = os.path.join(self.clevr_root,self.split,ann['image_filename'])
