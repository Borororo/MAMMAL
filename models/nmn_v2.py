...
class NMN(nn.Module):
    def __init__(self, cfg):
        ...
        self.transform_2nd_CA =  BertSelfAttention(config_attention, is_cross_attention=True,only_attn=True)
    ...
    def Find(self,control_state,question_attn,question_hidden,question_mask,image_hidden,image_mask, att_stack, stack_ptr, mem_in):
        ...
        # print(f'image hidde {image_hidden.shape}, mask{image_mask.shape}')
        # print(f'question attn {question_attn.shape}, hidden {question_hidden.shape},mask {question_mask.shape}')

        weighted_question_hidden = question_attn * question_hidden
        outputs = self.find_CA(image_hidden,attention_mask = image_mask.unsqueeze(1).unsqueeze(1),encoder_hidden_states=weighted_question_hidden,encoder_attention_mask =question_mask.unsqueeze(1).unsqueeze(1),output_attentions=True)
        att_out = outputs[1]
        att_out_scores = outputs[2]
        print(f"A output attention {att_out.shape} scores {att_out_scores.shape}")
        print(torch.sum(att_out,-1))
        # attn probs. shape [B, head number, Image patch, seq length] 如 [16,12,576,13]
        # 这里直接将每条head，对应的所有span分数进行求和，然后再softmax。

        outputs = self.find_CA(question_hidden,attention_mask = question_mask.unsqueeze(1).unsqueeze(1),encoder_hidden_states=image_hidden,encoder_attention_mask =image_mask.unsqueeze(1).unsqueeze(1),output_attentions=True)
        att_out = outputs[1]
        att_out_scores = outputs[2]
        print(f"B output attention {att_out.shape} scores {att_out_scores.shape}")
        print(torch.sum(att_out,-1))
        # TODO: 是否这里加入文本的权重分数？ 也就是，输入不再是加权的文本信息输入，而是放在后面。
        ...
        # print(f"final attention {att_out.shape} ")

        # Push to stack
        # attn out shape : [B, image patch, 1]
        stack_ptr = _move_ptr_fw(stack_ptr, self.cfg)
        # print(f"output stack_ptr {stack_ptr.shape} att_stack {att_stack.shape}")
        # output stack_ptr torch.Size([16, 8]) att_stack torch.Size([16, 576, 8])
    ...
    def Transform(self,control_state,question_attn,question_hidden,question_mask,image_hidden,image_mask, att_stack, stack_ptr, mem_in):
        ...
        # print(f" soft_att_in {soft_att_in.shape}")
        # print(torch.sum(soft_att_in,dim=1))
        ...
        last_output = self.transform_2nd_CA(weighted_image_hidden,attention_mask = image_mask.unsqueeze(1).unsqueeze(1),encoder_hidden_states =  intermediate_output[0], encoder_attention_mask =question_mask.unsqueeze(1).unsqueeze(1),output_attentions = True )
        ...
        att_out = last_output[1]
        att_out_scores = last_output[2]
        # attn probs. shape [B, head number, Image patch, seq length] 如 [16,12,576,13]
        att_out = torch.sum(torch.sum(att_out_scores,-1),1).unsqueeze(-1)
        # att_out = torch.nn.functional.softmax(torch.sum(torch.sum(att_out_scores,-1),1),-1).unsqueeze(-1)
        att_stack = _write_to_stack(att_stack, stack_ptr, att_out)
