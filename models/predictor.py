...
class TextGenerator(object):
    ...
    def translate_batch(self, encoder_inputs, do_sample=False):
        """
        ...
        """
        if do_sample:
            return self._fast_translate_batch(encoder_inputs, self.max_length, min_length=self.min_length, do_sample=do_sample)
        else:
            with torch.no_grad():
                return self._fast_translate_batch(encoder_inputs, self.max_length, min_length=self.min_length, do_sample=do_sample)
    ...
    def _fast_translate_batch(self,
        ...
        for each in results["scores"]:
            scores.append(each[0])
        for each in results["predictions"]:
            pred_ids.append(each[0])
        return pred_ids
