...
class BertSelfAttention(nn.Module):
    def __init__(self, config, is_cross_attention,only_attn=False):
        super().__init__()
        self.config = config
    ...
    def forward(
        ...
        # if is_cross_attention:
        #     print(f"initial  states {hidden_states.shape} corss {encoder_hidden_states.shape}")
        #     print(f"initial Q mask {attention_mask.shape}, KV {encoder_attention_mask.shape}")
        if is_cross_attention:
            key_layer = self.transpose_for_scores(self.key(encoder_hidden_states))

            value_layer = self.transpose_for_scores(self.value(encoder_hidden_states)) if not self.only_attn else None

            attention_mask = encoder_attention_mask
        elif past_key_value is not None:
            key_layer = self.transpose_for_scores(self.key(hidden_states))
            key_layer = torch.cat([past_key_value[0], key_layer], dim=2)
            value_layer = self.transpose_for_scores(self.value(hidden_states)) if not self.only_attn else None
            value_layer = torch.cat([past_key_value[1], value_layer], dim=2) if not self.only_attn else None
        else:
            key_layer = self.transpose_for_scores(self.key(hidden_states))
            value_layer = self.transpose_for_scores(self.value(hidden_states)) if not self.only_attn else None
        ...
        # if is_cross_attention:
        #     print(f"final attention {attention_scores.shape} mask {attention_mask.shape}")
...
class ModuleLayer(nn.Module):
    def __init__(self, config,is_cross_attention=True):
        ...
        self.seq_len_dim = 1
        self.attention = BertAttention(config,is_cross_attention=is_cross_attention)
