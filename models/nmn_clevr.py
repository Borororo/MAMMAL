...
class NMN(nn.Module):
    def __init__(self, cfg):
        ...
        self.find_conv = nn.Conv2d(cfg['internal_dim'], 1, 1)
        # transform
    ...
    def Find(self, kb_batch, att_stack, stack_ptr, mem_in, control_state):
        ...
        # print(f"kb_batch:{kb_batch.shape}")
        # print(f"elt_prod:{elt_prod.shape}")
    ...
    def Transform(self, kb_batch, att_stack, stack_ptr, mem_in, control_state):
        ...
        att_stack = _write_to_stack(att_stack, stack_ptr, att_out)
        # print(torch.nn.functional.softmax(att_out.view(att_out.size(0), self.H*self.W),-1))
        # print(torch.max(torch.nn.functional.softmax(att_out.view(att_out.size(0), self.H*self.W),-1),-1))
        # print(torch.min(torch.nn.functional.softmax(att_out.view(att_out.size(0), self.H*self.W),-1),-1))
        return (
            att_stack,
            stack_ptr,
            self.get_mem_zero(kb_batch.size(0), kb_batch.device),
