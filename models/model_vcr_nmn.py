...
class ALBEF(nn.Module):
    ...
    def forward(self, image, question, answer=None, alpha=0,k=None,weights=None, train=True):
        ...
        for i in range(self.steps):
            ...
            # print(f'step{i}')
            # print(_spatial_softmax(scores))
            # print(torch.max(_spatial_softmax(scores),-1))
            # print(torch.min(_spatial_softmax(scores),-1))
            # print(scores.shape)
            # print(scores)
            image_attns.append((att_stack * stack_ptr[:, None]).sum(-1))
            # print("*"*100)
            # print(image_attns[0].shape)
            # print(stack_ptr)
        ...
        if self.config['decoder_method'] == "planA":
            ...
            #B,H,W,D
            # exit()
            last_stack_image_hidden = image_attns[-1].view(batch_size,patch_size).unsqueeze(-1)* nocls_image_embeds.view(batch_size,patch_size,image_embeds.size(-1))
            #转化为B,H*W,1  B,H*W,D
    ...
    def rank_answer(self, question_states, question_atts, answer_ids, answer_atts, k=1):
