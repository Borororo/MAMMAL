...
class ALBEF(nn.Module):
    def __init__(self,
        ...
        self.cls_head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(self.text_encoder.config.hidden_size, NUM_CLASSES)
        )
    ...
    def forward(self, image, text, targets, alpha=0, train=True):
        ...
        if train:
            ...
            if self.distill:
                ...
                loss = (1 - alpha) * self.bce_loss(prediction, targets)*NUM_CLASSES - alpha*torch.sum(
                    F.log_softmax(prediction, dim=1)*F.softmax(prediction_m, dim=1),dim=1).mean()
            else:
                loss = self.bce_loss(prediction, targets)*NUM_CLASSES
            return loss
