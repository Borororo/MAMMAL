...
from models.model_vqa_clip_nmn import ALBEF
...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config,do_two_optim=False,do_three_optim=False):
    ...
    for i, (image, question, answer, weights, n, layout_label, soft_answer_label) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        outputs = model(image, question_input, answer_input, train=True, alpha=alpha, k=n, weights=weights)
        loss = torch.tensor(0.0, device=device, dtype=torch.float) 

        layout_loss = None
        vqa_loss = compute_vqa_softloss(config,outputs['answer_logits'],soft_answer_label)
        loss += vqa_loss
