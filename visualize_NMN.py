...
def visualize_vqa(model, data_loader, tokenizer,device,config,args):
    ...
    for n, (image, question, question_id,image_path) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        visualize_stepwise(outputs,image_path,args.vis_dir,question,question_id,question_input,clean_answer_list,tokenizer,module_names,predicts)


def visualize_stepwise(outputs,image_path,vis_root,question,question_id,question_input,answer_list,tokenizer,module_names,ans):
    ...
    print(f"image attn {torch.sum(outputs['iattns'],dim=[-1,-2])}")
